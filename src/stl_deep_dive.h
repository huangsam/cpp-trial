...
class STLDeepDive {
 public:
  ...
  // Map operations
  static std::map<std::string, int> count_words(const std::vector<std::string>& words);
  static std::string most_frequent_word(const std::map<std::string, int>& word_count);
  ...
  static std::vector<int> transform_multiply(const std::vector<int>& numbers, int factor);

  // Advanced: using algorithms with lambdas
  static std::vector<std::string> filter_long_words(const std::vector<std::string>& words, size_t min_length);
};
